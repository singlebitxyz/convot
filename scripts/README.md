...
This directory contains SQL scripts for setting up the Convot database schema.
...
### 2. `setup-convot-schema.sql`

Sets up all tables, indexes, RLS policies, and helper functions required for the Convot chatbot platform.
...
3. **Run Convot Schema Script**
    - Open `setup-convot-schema.sql`
    - Copy and paste the entire content
    - Click **Run** or press `Ctrl+Enter` (Windows/Linux) or `Cmd+Enter` (Mac)
...
supabase db execute -f scripts/setup-convot-schema.sql
```
...
psql "postgresql://postgres:[PASSWORD]@db.[PROJECT-REF].supabase.co:5432/postgres" -f scripts/setup-convot-schema.sql
```
...
### Tables Created by `setup-convot-schema.sql`
